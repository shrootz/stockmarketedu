...
public class RankByProfitableSale implements RankStudents{
	...
	public ArrayList<Student> returnRanking(ArrayList<Student> students) {
		...
		for(int i =0 ; i <rankedStudents.size(); i++){
			...
			int pos = i; 
			for(int j = i-1; j>=0; j--){
				if(rankedStudents.get(j).getMaxProfitableSale() > current.getMaxProfitableSale()){ //shift the current elements as long as you need to
